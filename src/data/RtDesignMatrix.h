...
public:
  ...
  void setTemplateDataID(const RtDataID &id) { templateDataID = id; }

  // set the number of measurements
  // NOTE: must be called before build()
  void setNumMeas(unsigned int _numMeas) { numMeas = _numMeas; }

  // set the tr
  // NOTE: must be called before build()
  void setTR(double _tr) { tr = _tr; }
  ...
  unsigned int getNumRows() { return rows(); }
  ...
  // get event duration
  unsigned int getMaxTrendOrder() { return maxTrendOrder; }
