...
vtkStandardNewMacro(FrColormapLayer);


FrColormapLayer::FrColormapLayer() 
...
// Accessors / Modifiers
void FrColormapLayer::SetInput(vtkImageData* data){
    if(m_cmFilter){
        m_cmFilter->SetInput(data);
    }
}

vtkImageData* FrColormapLayer::GetInput(){
...
void FrColormapLayer::SetColormapSettings(FrColormapSettings& settings){
...
void FrColormapLayer::GetColormapSettings(FrColormapSettings& settings){
    if(!m_cmFilter) return;
...
void FrColormapLayer::SetTbcSettings(FrTbcSettings& settings){
...
void FrColormapLayer::GetTbcSettings(FrTbcSettings& settings){
    if(!m_tbcFilter) return;
...
void FrColormapLayer::SetOpacity(double value){
    if(m_actor){
        m_actor->SetOpacity(value);
    }
}

double FrColormapLayer::GetOpacity(){
    double result = 0.0;
    if(m_actor){
...
void FrColormapLayer::SetVisibility(bool value){
    if(m_actor){
        m_actor->SetVisibility(value ? 1 : 0);
    }
}
bool FrColormapLayer::GetVisibility(){
    bool result = false;
    if(m_actor){
...
// Update methods
void FrColormapLayer::UpdateColormap(){
...
void FrColormapLayer::UpdateTbc(){
