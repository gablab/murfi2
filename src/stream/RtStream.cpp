...
RtStreamComponent *RtStream::buildStreamComponent(const string &type,
  ...
  else if (type == RtSlideWinCor::moduleString) { // voxel sliding window
                                                  // correlation
  ...
  else if (type == RtCurrentActivation::moduleString) { // voxel current
                                                        // activation
    ACE_NEW_NORETURN(sc, RtCurrentActivation());
  }
  else if (type == RtSingleImageCor::moduleString) { // voxel single image
                                                     // correlation
...
void RtStream::buildOutputNames(TiXmlElement *module, vector<string> &names) {
  ...
  while ((child = module->IterateChildren("output", child))) {
    ...
    // TODO consider using child iterators here
    string outid = "output:";
