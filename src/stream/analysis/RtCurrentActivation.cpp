...
bool RtCurrentActivation::processOption(const string &name, const string &text, const map<string, string> &attrMap) {
...
int RtCurrentActivation::process(ACE_Message_Block *mb) {
    ...
    currentActivation->getDataID().setRoiID(modelFitRoiID); //TODO need to declare/change this
	...
	return 0;
      ...
      //cout << "RtCurrentActivation:process: got beta " << j << " " << endl;
    ...
    //cout << "RtCurrentActivation:process: got residual" << endl;
	...
	if (!design->isColumnOfInterest(j)) { // TODO change this loop to use isNotColumnOfInterest, will iterate through non-columns of interest
