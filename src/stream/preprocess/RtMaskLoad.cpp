...
int RtMaskLoad::process(ACE_Message_Block *mb) {
  ...
  maskLoad->setFilename(getExperimentConfig().get("study:maskDir").str() + filename);
  ...
  if(align) { 
    ...
    if(status == FSL_JOB_FINISHED) {
      maskLoad->setFilename(getExperimentConfig().getSeriesXfmOutputFilename(seriesNum,
					      maskLoad->getFilename()));
    }
    else {
  ...
  if(needsInit && save) {
    maskLoad->setFilename(getExperimentConfig().getSeriesMaskFilename(seriesNum,roiID));
    maskLoad->save();
