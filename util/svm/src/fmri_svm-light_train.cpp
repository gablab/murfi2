...
#include"fmri_svm-light_train.h"
...
int main(int argc, char **argv) {
  ...
  // train
  KERNEL_CACHE *kernelCache = NULL;
  if(parms.kernelParm.kernel_type != LINEAR) {
    kernelCache = kernel_cache_init(parms.numExamples,
				    parms.learnParm.kernel_cache_size);
  }
  
  long maxWnum = parms.docs[0]->fvec->words[parms.numWords-1].wnum;
  svm_learn_classification(parms.docs, parms.labels,
			   parms.numExamples,
			   maxWnum,
			   &parms.learnParm, &parms.kernelParm,
			   kernelCache,
			   &parms.model,
			   parms.alpha);
  ...
  if(parms.kernelParm.kernel_type != LINEAR) {
    kernel_cache_cleanup(kernelCache);
  }
  else if(!parms.weightFile.empty()) {
    add_weight_vector_to_linear_model(&parms.model);
    float *weights = convertDoubleArrayToFloatArray(parms.model.lin_weights,
						    maxWnum);
...
void printUsage(string executename) {
  cout << executename << " [options] -- [svm-light options]" << endl
       ...
       << "EXAMPLE:" << endl
       << "fmri_svm-light_train -t fmri_run.nii -l fmri_design.tl \\" << endl
       << "                     -m mask.nii -s model.svm -w weights.nii -- -v 1 -x 1" << endl
       << endl << flush;
...
Parms parseArgs(int argc, char **argv) {
  ...
  int ind = 1;
  for(; !done; ind+=2) {
    opt = getopt_long (argc, argv, "-t:l:m:M:f:p:cs:w:v?",
		       NULL, &option_index);
    if(opt == -1) {
      break;
    }
