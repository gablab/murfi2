			      ...
			      KERNEL_PARM *kparm)
{
  ...
  for(i=0;i<sample.n;i++)     /* find highest class label */
    if(sparm->num_classes < (sample.examples[i].y.cls+0.1)) 
      sparm->num_classes=sample.examples[i].y.cls+0.1;
...
// ohinds:
// added to avoid calling read_input_parms
void init_train(STRUCT_LEARN_PARM *struct_parm,
  ...
  verbosity=1;/*verbosity for svm_light*/
				    ...
				    STRUCT_LEARN_PARM *sparm)
{
  ...
  LABEL y;
  DOC doc;
  long cls,bestclass=-1,first=1,j;
  ...
  for(cls=1;cls<=sparm->num_classes;cls++) {
    y.cls=cls;
...
double      loss(LABEL y, LABEL ybar, STRUCT_LEARN_PARM *sparm)
{
  ...
  if(sparm->loss_function == 0) { /* type 0 loss: 0/1 loss */
    if(y.cls == ybar.cls)     /* return 0, if y==ybar. return 100 else */
      return(0);
    else
      return(100);
  }
  if(sparm->loss_function == 1) { /* type 1 loss: squared difference */
    return((y.cls-ybar.cls)*(y.cls-ybar.cls));
  }
  else {
